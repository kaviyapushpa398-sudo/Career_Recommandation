...
Phase 3: GitHub API Integration, Repository Analysis, Activity Scoring.
"""
...
@login_required
def github_page():
    """Renders the GitHub Analysis page (Phase 3)."""
    return render_template("github.html", full_name=session.get("full_name"))
...
# =====================================================
# Routes: API (GitHub Analysis) - Phase 3
# =====================================================
...
def api_github_analyze():
    """
    Triggers a fresh GitHub analysis for the logged-in user.

    Expects JSON: { "github_username": "octocat" }

    Pipeline:
      1. Call github_analyzer.analyze_github_user() — hits GitHub API.
      2. UPSERT results into github_analysis table.
      3. Replace github_languages rows for this user.
      4. Return full analysis as JSON.
    """
    user_id = session["user_id"]
    data = request.get_json(silent=True) or {}
    ...
    # ── Call the analyzer (may take several seconds) ──────────────────────────
    result = analyze_github_user(github_username)
    ...
    try:
        ...
        # UPSERT github_analysis (one row per user)
        cursor.execute(
            """
            INSERT INTO github_analysis (
                user_id, github_username, github_name, github_bio,
                github_avatar_url, github_profile_url, github_location,
                account_created_at, public_repos, total_stars, total_forks,
                total_watchers, followers, following, public_gists,
                activity_score, repo_score, community_score, overall_score,
                top_languages_json, top_repos_json
            )
            ...
            ON DUPLICATE KEY UPDATE
                ...
                github_location     = VALUES(github_location),
                ...
                top_repos_json      = VALUES(top_repos_json),
                analyzed_at         = CURRENT_TIMESTAMP
            """,
            (
                user_id,
                d["github_username"],
                d["github_name"]        or None,
                d["github_bio"]         or None,
                d["github_avatar_url"]  or None,
                d["github_profile_url"] or None,
                d["github_location"]    or None,
                d["account_created_at"] or None,
                d["public_repos"],
                d["total_stars"],
                d["total_forks"],
                d["total_watchers"],
                ...
                d["community_score"],
                d["overall_score"],
                json.dumps(d["top_languages"]),
                json.dumps(d["top_repos"]),
            )
        )

        # Replace per-language rows for this user
        cursor.execute("DELETE FROM github_languages WHERE user_id = %s", (user_id,))
        for row in d["language_table"]:
            cursor.execute(
                """
                INSERT INTO github_languages (user_id, language, bytes_count, repo_count, percentage)
                VALUES (%s, %s, %s, %s, %s)
                """,
                (user_id, row["language"], row["bytes"], row["repo_count"], row["percentage"])
            )
        ...
        return jsonify({"success": False, "message": "Could not save analysis results."}), 500
...
@app.route("/api/github/analysis", methods=["GET"])
@login_required
def api_github_get_analysis():
    """
    Returns the most recent stored GitHub analysis for the logged-in user.
    Also returns the language breakdown from github_languages.
    ...
    try:
        ...
        cursor.execute(
            """
            SELECT github_username, github_name, github_bio, github_avatar_url,
                   github_profile_url, github_location, account_created_at,
        ...
        # Parse JSON columns
        row["top_languages"] = json.loads(row.pop("top_languages_json") or "[]")
        row["top_repos"]     = json.loads(row.pop("top_repos_json")     or "[]")

        # Stringify timestamp for JSON
        if row.get("analyzed_at"):
            row["analyzed_at"] = row["analyzed_at"].isoformat()

        # Convert Decimal scores to float
        for key in ("activity_score", "repo_score", "community_score", "overall_score"):
        ...
        cursor.execute(
            """
            SELECT language, bytes_count, repo_count, percentage
        ...
        for lr in lang_rows:
            lr["percentage"] = float(lr["percentage"])
        row["language_table"] = lang_rows

        return jsonify({"success": True, "data": row}), 200

    except Exception as e:
        print(f"[GITHUB GET ANALYSIS ERROR] {e}")
        return jsonify({"success": False, "message": "Could not fetch analysis."}), 500
...
@app.route("/api/github/analysis", methods=["DELETE"])
@login_required
def api_github_delete_analysis():
    """Removes the stored GitHub analysis for the logged-in user."""
    user_id = session["user_id"]
    conn = get_db_connection()
    if conn is None:
        return jsonify({"success": False, "message": "Database connection failed."}), 500
    ...
    try:
        cursor = conn.cursor()
        cursor.execute("DELETE FROM github_analysis   WHERE user_id = %s", (user_id,))
        cursor.execute("DELETE FROM github_languages  WHERE user_id = %s", (user_id,))
        conn.commit()
        return jsonify({"success": True, "message": "GitHub analysis cleared."}), 200

    except Exception as e:
        print(f"[GITHUB DELETE ERROR] {e}")
        return jsonify({"success": False, "message": "Could not delete analysis."}), 500
